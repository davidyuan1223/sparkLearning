...
package object config {
  ...
  private [spark] val RESOURCES_DISCOVERY_PLUGIN =
    ConfigBuilder("spark.resources.discoveryPlugin")
      .doc("Comma-separated list of class names implementing" +
        ...
        "This if for advanced users to replace the resource discovery class with a " +
        "custom implementation. Spark will try each class specified until one of them " +
        "return the resource information for that resource. It tries the discovery " +
  ...
  private [spark] val DRIVER_RESOURCES_FILE=
    ConfigBuilder("spark.driver.resourcesFile")
      .internal()
      .doc("Path to file containing the resources allocated to the driver. " +
  ...
  private [spark] val DRIVER_CORES =
    ConfigBuilder("spark.driver.cores")
  ...
  private [spark] val DRIVER_MEMORY =
    ConfigBuilder(SparkLauncher.DRIVER_MEMORY)
  ...
  private [spark] val DRIVER_MEMORY_OVERHEAD_FACTOR =
    ConfigBuilder("spark.driver.memoryOverheadFactor")
      ...
      .version("3.3.0")
      .doubleConf
      .checkValue(factor => factor >0 ,"Ensure that memory overhead is a double greater than 0")
  ...
  private [spark] val EVENT_LOG_DIR =
    ConfigBuilder("spark.eventLog.dir")
      .version("1.0.0")
      .stringConf
  ...
  private [spark] val EVENT_LOG_ROLLING_MAX_FILE_SIZE =
    ConfigBuilder("spark.eventLog.rolling.maxFileSize")
      ...
      .checkValue(_>=ByteUnit.MiB.toBytes(10), "Max file size of event log should be configured to be at least 10 MiB.")
  ...
  private [spark] val EXECUTOR_PROCESS_TREE_METRICS_ENABLED =
    ConfigBuilder("spark.executor.processTreeMetrics.enabled")
      .doc("Whether to collect process tree metrics (from the /proc filesystem) when collecting executor metrics.")
      .version("3.0.0")
      .booleanConf
      .createWithDefault(false)
  ...
  private [spark] val EXECUTOR_MEMORY_OVERHEAD=
    ConfigBuilder("spark.executor.memoryOverhead")
      .doc("The amount of non-heap memory to be allocated per executor, in MiB unless otherwise specified.")
      .version("2.3.0")
      .bytesConf(ByteUnit.MiB)
      .createOptional
