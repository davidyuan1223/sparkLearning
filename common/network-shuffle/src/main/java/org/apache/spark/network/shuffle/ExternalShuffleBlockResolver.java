...
public class ExternalShuffleBlockResolver {
    ...
    ExternalShuffleBlockResolver(TransportConf conf,File registeredExecutorFile,Executor directoryCleaner)throws IOException{
        ...
        CacheLoader<String ,ShuffleIndexInformation> indexCacheLoader =
                new CacheLoader<String, ShuffleIndexInformation>() {
                    @Override
                    public ShuffleIndexInformation load(String filePath) throws Exception {
                        return new ShuffleIndexInformation(filePath);
                    }
                };
        shuffleIndexCache= CacheBuilder.newBuilder()
                .maximumWeight(JavaUtils.byteStringAsBytes(indexCacheSize))
                .weigher((Weigher<String,ShuffleIndexInformation>)(filePath,indexInfo) -> indexInfo.getRetainedMemorySize())
        ...
        if (db != null) {
            logger.info("Use {} as the implementation of {}",dbBackend,Constants.SHUFFLE_SERVICE_DB_BACKEND);
    ...
    public ManagedBuffer getRddBlockData(String appId,String execId,int rddId,int splitIndex){
        ...
        return getDiskPersistedBlockData(executor,rddId,splitIndex);
    }

    public void applicationRemove(String appId,boolean cleanupLocalDirs){
        logger.info("Application {} remvoed, cleanupLocalDirs = {}",appId,cleanupLocalDirs);
    ...
    private ManagedBuffer getSortBasedShuffleBlockData(ExecutorShuffleInfo executor, int shuffleId, long mapId,int startReduceId,int endReduceId){
        String indexFilePath =
                ExecutorDiskUtils.getFilePath(executor.localDirs,executor.subDirsPerLocalDir,"shuffle_"+shuffleId+"_"+mapId+"_0.index");
        try {
            ...
            return new FileSegmentManagedBuffer(conf,new File(ExecutorDiskUtils.getFilePath(executor.localDirs,executor.subDirsPerLocalDir,"shuffle_"+shuffleId+"_"+mapId+"_0.data")),
    ...
    public ManagedBuffer getDiskPersistedBlockData(ExecutorShuffleInfo executor, int rddId,int splitIndex){
