...
public class TransportServer implements Closeable {
    ...
    public TransportServer(
        ...
        if (conf.sharedByteBufAllocators()) {
            this.pooledByteBufAllocator= NettyUtils.getSharedPooledByteBufAllocator(
                    conf.preferDirectBufsForSharedByteBufAllocators(),true
            );
        }else {
            this.pooledByteBufAllocator=NettyUtils.createPooledByteBufAllocator(
    ...
    private void init(String hostToBind, int portToBind){
        ...
        this.metrics=new NettyMemoryMetrics(pooledByteBufAllocator,conf.getModuleName()+"-server",conf);
        if (conf.backLog()>0) {
        ...
        bootstrap.childHandler(new ChannelInitializer<SocketChannel>() {
            ...
            protected void initChannel(SocketChannel socketChannel) throws Exception {
                logger.debug("New connection accepted for remote address {}.",socketChannel.remoteAddress());
                RpcHandler rpcHandler=appRpcHandler;
                for (TransportServerBootstrap bootstrap : bootstraps) {
                    rpcHandler=bootstrap.doBootstrap(socketChannel,rpcHandler);
                }
                context.initializePipeline(socketChannel,rpcHandler);
            }
        });
        InetSocketAddress address = hostToBind==null ?new InetSocketAddress(portToBind):new InetSocketAddress(hostToBind,port);
